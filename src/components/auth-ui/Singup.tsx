...
import { UserType, UserZodSchema } from "@/types/auth";
...
export function SignUpForm({
  ...
  const submitForm = async (data: UserType) => {
    const res = await client.user.signUp.$post(data);
    if (!res.ok) {
            ...
            <div className="my-4">
              <Button className="text-xs w-full rounded-xl" type="submit">
                {rhf.formState.isSubmitting ? (
                  <Loader2 size={30} className="animate-spin duration-370" />
                ) : (
