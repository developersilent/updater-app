...
import { AlertCircle, Book, BookOpenCheck } from "lucide-react";
...
import { useAddChapter } from "@/querys/use-chapter";
...
export function ChapterPopupCreate({ subjectId }: { subjectId: string }) {
  ...
  const handleSubmit = async (data: ChapterInput) => {
    ...
    if (!res?.success) {
      ToastError(res?.message || "Failed to add chapter. Please try again.");
      return;
    }
    ToastSuccess(res.message || "Chapter added successfully!");
    rhf.reset();
    setOpen(false);
  };
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="lg" className="border cursor-pointer">
          <BookOpenCheck className="text-purple-400 size-5" />
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center">
            ...
            <span className="align-middle ml-2 text-sm">Add Chapter</span>
          </DialogTitle>
