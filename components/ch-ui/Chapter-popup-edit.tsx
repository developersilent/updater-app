...
import { AlertCircle, Book, Edit } from "lucide-react"
...
const chapterSchema = z.object({
    ...
    status: z.enum(["COMPLETED", "DOING_IT_CURRENTLY", "NOT_STARTED"] as const).optional().default("NOT_STARTED"),
    compeleted_percentage: z.string().optional().default("0%")
})

type ChapterInput = z.input<typeof chapterSchema>;

export function ChapterPopupEdit(props: ChapterInput & { subjectId: string, chapterId: string }) {
    
    const router = useRouter();
    const { subjectId, chapter_name, chapter_number, compeleted_percentage, status, chapterId } = props;
    ...
    const handleSubmit = async (data: ChapterInput) => {
        try {
            const parsed = chapterSchema.parse(data);
            const res = await api.chapter.edit({chapterId}).put({
                ...parsed,
                subject_id: subjectId
            })
            if (!res.response.ok || res.data?.error) {
                ToastError(res.data?.message || "Failed to update chapter. Please try again.");
                return;
            }
            ToastSuccess(res.data?.message || "Chapter updated successfully!");
            setOpen(false);
            rhf.reset();
            router.refresh();
        } catch {
            ToastError("Failed to update chapter. Please try again.");
        }
    }
    return (
            ...
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle className="flex items-center">
                        <Book className="size-7 text-purple-400 animate-pulse" /> <span className="align-middle ml-2 text-sm">Editing Chapter</span>
                    </DialogTitle>
                                        ...
                                        <FormControl>
                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                ...
                                                <SelectContent>
                                                    ...
                                                    <SelectItem value="DOING_IT_CURRENTLY">Doing It Currently</SelectItem>
